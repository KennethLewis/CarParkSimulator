...
public class CarPark {
		...
		for (int i = 0; i < spaces.size(); i++) {
			...
			if (spaces.get(i).isParked() == false)
				throw new SimulationException(
						"One or more departing Vehicles are"
	...
	public boolean carParkFull() {
		...
		if (this.getNumCars() + this.getNumMotorCycles() >= this.maxCarSpaces
	...
	public void enterQueue(Vehicle v) throws SimulationException,
		...
		if (queue.size() != maxQueueSize) {
			...
			// Time passed through to archiveQueue method should be the current
			// time
	...
	public void exitQueue(Vehicle v, int exitTime) throws SimulationException,
		...
		if (v.isQueued() == false) {
			throw new SimulationException(
					"Vehicle is currently not in the CarPark Queue!\n"
							+ v.toString());
		} else {
			v.exitQueuedState(exitTime);

			// We aren't actually removing from the queue yet (to avoid
			// iteration issues).
	...
	public void parkVehicle(Vehicle v, int time, int intendedDuration)
		...
		if (v instanceof Car && ((Car) v).isSmall() == false
				&& spacesAvailable(v) == true) {
			...
			throw new SimulationException(
					"Vehicle cannot park, as there are no available"
	...
	public void processQueue(int time, Simulator sim) throws VehicleException,
		...
		// We're removing the queue elements now, since we can't do it
		// mid-iteration
	...
	public boolean spacesAvailable(Vehicle v) {
		...
		// Math.max used to convert any negatives into 0s (negatives occurring
		// when there are

		// Calculate the number of motorcycles in small car spaces (aka
		// overflow)
		// Negative values mean that there's still space left
		int motorCycleOverflow = this.getNumMotorCycles()
				- this.maxMotorCycleSpaces;

		// If there's cycles overflowing, the normal motorcycle spaces must be
		// full
		...
		// so if there's no overflow, there won't be any effect on our later
		// equations
		motorCycleOverflow = Math.max(0, motorCycleOverflow);

		int smallCarOverflow = this.getNumSmallCars() + motorCycleOverflow
		...
		if ((this.getNumCars() - this.getNumSmallCars()) >= (this.maxCarSpaces
				- this.maxSmallCarSpaces - smallCarOverflow)) {
			normalCarFull = true;
		}

		// Now that we've calculated which spaces are full, let's return our
		// values
		if (this.carParkFull()) {
			return false;
		} else if (motorCycleFull && smallCarFull && v instanceof MotorCycle) {
			return false;
		} else if (smallCarFull && normalCarFull && v instanceof Car
				&& ((Car) v).isSmall() == true) {
			return false;
		} else if (normalCarFull && v instanceof Car
				&& ((Car) v).isSmall() == false) {
			return false;
		} else {
			return true;
	...
	/***
	 * Helper method used when processing new vehicles. Parks, archives or
	 * queues. Also adds transition text to the status
	 * 
	 * @param v
	 *            The vehicle to process
	 * @param time
	 ...
	 * @Author Thomas McCarthy
	 */
	private void handleNewVehicle(Vehicle v, int time, Simulator sim)
			throws SimulationException, VehicleException {
	...
	public void unparkVehicle(Vehicle v, int departureTime)
		...
		if (!spaces.contains(v)) {
			throw new SimulationException(
					"Cannot unpark vehicle as it is not in the carpark");
		}
	 ...
	 * 
	 * @param v
	 *            Vehicle helping to make sure what addition to the numbers is
	 *            being required.
	 * @author Ken Lewis
	 */
	 ...
	 * 
	 * @param v
	 *            Vehicle helping to make sure what subtraction to the numbers
	 *            is being required.
	 * @author Ken Lewis
	 */
